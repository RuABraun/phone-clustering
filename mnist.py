...
class NetX(nn.Module):
    def __init__(self, num_heads=5):
        ...
        self.conv1 = nn.Conv2d(1, 64, 5, 2, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(64, 64, 5, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(64)
        self.conv3 = nn.Conv2d(64, 64, 5, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(64)
        self.conv4 = nn.Conv2d(64, 256, 4, 1, bias=False)
    ...
    def forward(self, x):
        ...
        # x = F.softmax(self.fc2(x_prefinal), dim=1)
...
def report_gradnorms(model):
    ...
    for name, p in model.named_parameters():
        ...
        if gradnorm < minnorm:
            minnorm = gradnorm
            Wname = name
    logger.info('minnorm: %s %f' % (Wname, minnorm))
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            ref_targs.append(target)

    out_targs = [torch.cat(out_targs[i]).cpu() for i in range(5)]
...
def main():
    ...
    parser.add_argument('--lr', type=float, default=0.0045, metavar='LR',
